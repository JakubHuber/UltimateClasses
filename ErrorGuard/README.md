...
|Sub   |EnableLogErrorsToFile(ByVal **logFileName** As String, **CreateLogInProjectFolder** As Boolean)   |Enables loging into text file, with or withaout extension.  If **logFileName** is not provided errors are written to default *ErrorGuard.log*.  **CreateLogInProjectFolder** creates log folder in root project folder. If false then folder is created in user LOCALAPPDATA folder. Sub displays in Immidiate Window where log will be saved.  |
